...
def calculate_correlations(filename):
    ...
    for i in range(1, len(column_names) - 1):
        ...
        correlation_pearsonr, _ = pearsonr(pork_prices, column_data)
        correlation_spearmanr, _ = spearmanr(pork_prices, column_data)
        correlation = (correlation_pearsonr + correlation_spearmanr) / 2
        correlations.append((column_name, correlation))

    # 计算时间列与猪肉价格的皮尔逊相关系数和斯皮尔曼相关系数的平均值
    time_corr_pearsonr, _ = pearsonr(time_values, pork_prices)
    time_corr_spearmanr, _ = spearmanr(time_values, pork_prices)
    time_corr = (time_corr_pearsonr + time_corr_spearmanr) / 2
...
def plot_correlations(correlations):
    ...
    for column, correlation in correlations:
        print(f"猪肉价格与{column} 的皮尔逊积矩相关系数和斯皮尔曼等级相关系数的平均值：{format(correlation, '.20f')}")
    ...
    plt.ylabel("相关系数和秩相关系数的平均值")
    plt.title("猪肉价格与各因素的皮尔逊积矩相关系数和斯皮尔曼等级相关系数的平均值")
